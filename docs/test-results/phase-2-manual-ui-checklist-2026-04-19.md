...
- `PASS` Activity: live Coinbase account-event ledger is present with filters, empty-state handling, and event details.
...
- `PASS` Alerts: recent alerts, rule rows, and local preference controls render without implying autonomous trading alerts.
- `PASS` Configuration: Coinbase and OpenAI connection setup is simplified around credentials and refresh actions.
...
- The most mature read-only surfaces are now `Overview`, `Assets`, `Portfolio`, and `Activity`.
